...
import { User, Event, EventResponse } from "./db/models";
import { addUser, getAllEvent } from "./services/api";
import { updateResults } from "./botActions/getlastupdate";
import { generateDateTimeSelector } from "./utils/calendar";
import { repeatRassilka, sendRassilkaMessageWithPhoto, } from "./utils/rassilka";
import { handleEventCreationCallback } from "./botActions/eventHandlersCallback";
...
bot.callbackQuery(/^change_event:(\d+)/, async (ctx) => {
    ...
    const keyboard = new InlineKeyboard()
        ...
        .text(newIndex < allEvents.length - 1 ? "Следующий 👉" : " ", `change_event:${newIndex + 1}`);
    try {
        const message = `
        Всем добрый вечер🌆🙏\n\n🧘‍♀️Приглашаю всех присутствующих на "<b>${event.title}</b>"🧘‍♀️\n\n<i>Когда?</i> <b>${new Date(event.event_time).toLocaleString("ru-RU", {
            day: "numeric",
            month: "numeric",
            weekday: "long",
            hour: "numeric",
            minute: "numeric",
        })}</b>\n\n<i>Где?</i> <b>${event.address}</b>\n\n<i>Что берем с собой?</i> <b>Прекрасное настроение, удобную одежду, водичку)</b>\n\n<i>Что нас ждет?</i><b>${event.description || "Подробности уточняются."}</b>\n\nЗаписаться можно через личные сообщения <b>@vslomalinafik</b> 💌
      `;
...
bot.callbackQuery(/^event_page:(\d+)/, async (ctx) => {
    ...
    let lastUpdate = null;
    let lastText = "";
    const results = await updateResults(ctx.callbackQuery.message?.message_id, lastText, ctx, bot, lastUpdate);
    const pages = results.responseText.split("\n\n---\n\n");
    const totalPages = pages.length;
    if (page < 0 || page >= totalPages) {
        return ctx.answerCallbackQuery("Некорректная страница.");
    }
    await ctx.api.editMessageText(ctx.chat.id, ctx.callbackQuery.message?.message_id, pages[page], {
        parse_mode: "HTML",
        reply_markup: new InlineKeyboard()
            .text(page > 0 ? "⬅️ Назад" : "", `event_page:${page - 1}`)
            .text(page < totalPages - 1 ? "Вперед ➡️" : "", `event_page:${page + 1}`),
    });
    await ctx.answerCallbackQuery();
});
bot.callbackQuery(/^sendrassilka:(\d+)$/, async (ctx) => {
    ...
    try {
      ...
      `;
        ...
        await ctx.answerCallbackQuery("Рассылка отправлена пользователям, которые не ответили.");
    }
    catch (error) {
        console.error(error);
...
bot.hears("Результаты", async (ctx) => {
    ...
    let lastUpdate = null;
    let lastText = "";
    const results = await updateResults(ctx.message.message_id, lastText, ctx, bot, lastUpdate);
    ...
    const getKeyboard = (page) => {
        const keyboard = new InlineKeyboard();
        if (page > 0)
            keyboard.text("⬅️ Назад", `event_page:${page - 1}`);
...
bot.hears("Ивенты", async (ctx) => {
    ...
    const sendEventMessage = async (index) => {
        ...
        const keyboard = new InlineKeyboard()
            ...
            .text("Повторная рассылка", `sendrassilka:${event.id}`)
            .text(index < allEvents.length - 1 ? "Следующий 👉" : " ", `change_event:${index + 1}`);
...
bot.on("message", async (ctx) => {
    ...
    if (!currentEvent)
        ...
        if (!currentEvent.address) {
            currentEvent.address = ctx.message.text;
            const now = new Date();
            const calendar = generateDateTimeSelector(now.getFullYear(), now.getMonth(), "date");
            await ctx.reply("Выберите дату:", { reply_markup: calendar });
            return;
        }
        if (!currentEvent.event_time && currentEvent.waitingForDate) {
        ...
        if (!currentEvent.image && currentEvent.waitingForPhoto) {
            if (ctx.message.photo && ctx.message.photo.length > 0) {
                ...
                try {
                    const event = await Event.create(currentEvent);
                    delete eventCreationData[ctx.from.id];
                    const message = `${event.invitation_message}\n\n${event.title}\n\nКогда?${new Date(event.event_time).toLocaleString("ru-RU", {
                        ...
                        hour: "numeric",
                        minute: "numeric",
                    })}\n\nГде?${event.address}\n\n${event.description || "Подробности уточняются."}Записаться можно через личные сообщения @vslomalinafik 💌
            `;
                    await sendRassilkaMessageWithPhoto(message, currentEvent.image, bot, event.id);
                }
                catch (error) {
                    console.error("Ошибка при создании ивента:", error);
                    await ctx.reply("Произошла ошибка при создании ивента. Пожалуйста, попробуйте снова.");
                }
            }
            else {
...
bot.on("callback_query", (ctx) => handleEventCreationCallback(ctx, eventCreationData));
