import { Event, EventResponse, User } from "../db/models";
...
export const updateResults = async (messageId, lastText, ctx, bot, lastUpdate) => {
    ...
    const responseTexts = events.map((event) => {
        ...
        responseText += `<b>Когда: ${new Date(event.event_time).toLocaleString("ru-RU", { day: "numeric", month: 'numeric', hour: "numeric", minute: "numeric" })}</b>\n\n`;
        const attending = event.EventResponses.filter((r) => r.response === "Пойду");
        const notAttending = event.EventResponses.filter((r) => r.response === "Не пойду");
