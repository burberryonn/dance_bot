...
export function generateDateTimeSelector(
  year: number,
  month: number,
  state: "date" | "hour" | "minute",
  hour?: number
) {
  ...
  if (state === "date") {
    ...
    keyboard
      .text(
        "⬅️",
        `change_month:${prevMonth.getFullYear()}-${prevMonth
          .getMonth()
          .toString()
          .padStart(2, "0")}`
      )
      .text(
        "➡️",
        `change_month:${nextMonth.getFullYear()}-${nextMonth
          .getMonth()
          .toString()
          .padStart(2, "0")}`
      );
